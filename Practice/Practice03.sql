...
-- 사번(employee_id) 오름차순 으로 정렬하세요. 부서가 없는 Kimberely(사번 178)은 표시하지 않습니다. (106건) 
select e.employee_id
	, e.first_name
	, e.salary
    , d.department_name
...
-- 부서가 없는 직원(Kimberely)도 표시합니다. 매니저가 없는 Steven도 표시합니다. (107명)
select e.employee_id
	, e.first_name
    , d.department_name
    , e.manager_id
...
-- 문제9-2. 
-- 문제9 에서 부서가 없는 직원(Kimberely)도 표시하지 않고 매니저가 없는 Steven도 표시하지 않습니다. (105명)
